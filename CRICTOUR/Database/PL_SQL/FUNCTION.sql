...
BEGIN
    ...
    LOOP
        TOTAL_POINTS := TOTAL_POINTS +
            (SELECT COALESCE(SUM(RUN_SCORED), 0) * 1 +      -- 1 POINT FOR EACH RUN SCORED
        			COALESCE(SUM(WICKET_TAKEN), 0) * 10 +   -- 10 POINTS FOR EACH WICKET TAKEN
        			COALESCE(SUM(RUN_GIVEN), 0) * -0.5 +    -- 0.5 POINT DEDUCTION FOR EACH RUN
...
BEGIN
    ...
    SELECT 
        COALESCE(SUM(RUN_SCORED), 0) * 1 +      -- 1 POINT FOR EACH RUN SCORED
        COALESCE(SUM(WICKET_TAKEN), 0) * 10 +   -- 10 POINTS FOR EACH WICKET TAKEN
        COALESCE(SUM(RUN_GIVEN), 0) * -0.5 +    -- 0.5 POINT DEDUCTION FOR EACH RUN
        COALESCE(SUM(MAIDEN_OVERS), 0) * 5 +    -- 5 POINTS PER MAIDEN OVER
        COALESCE(SUM(TOTAL_SIXES_HIT), 0) * 2 + -- 2 POINTS FOR EACH SIX HIT
